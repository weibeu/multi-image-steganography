...
class SteganoModel(nn.Module):
    ...
    def encrypt(self, cover_image_tensor, secret_image_tensor1, secret_image_tensor2, secret_image_tensor3):
        ...
        for param in self.decoder.parameters():
            param.requires_grad = False
        hidden_image = self.encoder(cover_image_tensor, secret_image_tensor1, secret_image_tensor2,
                                    secret_image_tensor3)
        return hidden_image
...
def encrypt(steganographer, cover_image_path, secret_image1_path, secret_image2_path, secret_image3_path):
...
def decrypt(steganographer, hidden_image_path):
...
async def lifespan(app_: FastAPI):
    ml_models["steganographer"] = get_model()
    yield
    ml_models.clear()
...
async def encrypt_(
    ...
    secret_image1: UploadFile = File(...),
    secret_image2: UploadFile = File(...),
    secret_image3: UploadFile = File(...),
):
    steganographer = ml_models['steganographer']

    cover_image_buffer = BytesIO(await cover_image.read())
    secret_image1_buffer = BytesIO(await secret_image1.read())
    secret_image2_buffer = BytesIO(await secret_image2.read())
    secret_image3_buffer = BytesIO(await secret_image3.read())

    cover_image_buffer.seek(0)
    secret_image1_buffer.seek(0)
    secret_image2_buffer.seek(0)
    secret_image3_buffer.seek(0)

    hidden_image_tensor = encrypt(
        steganographer,
        cover_image_buffer,
        secret_image1_buffer,
        secret_image2_buffer,
        secret_image3_buffer,
    )
    hidden_image = to_image(hidden_image_tensor)

    return JSONResponse(content=dict(hidden_image=b64encode(hidden_image)))
...
async def decrypt_(
    ...
    hidden_image_buffer = BytesIO(await hidden_image.read())
    hidden_image_buffer.seek(0)

    reveal_image1_tensor, reveal_image2_tensor, reveal_image3_tensor = decrypt(
        steganographer,
        hidden_image_buffer
    )

    reveal_image1 = to_image(reveal_image1_tensor)
    reveal_image2 = to_image(reveal_image2_tensor)
    reveal_image3 = to_image(reveal_image3_tensor)

    data = {
        "reveal_image1": b64encode(reveal_image1),
        "reveal_image2": b64encode(reveal_image2),
        "reveal_image3": b64encode(reveal_image3),
    }
